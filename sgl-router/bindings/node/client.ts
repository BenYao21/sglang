...
export class SglangClient {
  ...
  static async connect(endpoint: string, tokenizerPath: string): Promise<SglangClient> {
    ...
    if (request.stream) {
        ...
        return {
            [Symbol.asyncIterator]: async function* () {
                ...
                const callback = (err: Error | null, chunk: string) => {
                    if (err) {
                        error = err;
                    } else {
                        try {
  ...
  chatCompletionStream(request: ChatCompletionRequest, onChunk: (chunk: any) => void, onError: (err: any) => void): void {
      const jsonStr = JSON.stringify(request);
      this.inner.chatCompletionStream(jsonStr, (err: Error | null, chunkStr: string) => {
          if (err) {
              onError(err);
          } else {
